...
def main(args):
    ...
    train_u = reader.read_field('sol')[:args.ntrain,::args.r,::args.r].reshape(args.ntrain,-1)
    u_normalizer = GaussianNormalizer(train_u)

    # read the preprocessed dataset
    dataset_dir = args.dataset_dir
    file_path=os.path.join(dataset_dir, 'data_train.pt')
    train_loader = DataLoader(torch.load(file_path), batch_size= args.batch_size, shuffle=True)
    test_loader16 = DataLoader(torch.load(os.path.join(dataset_dir, 'data_test16.pt')), batch_size=args.batch_size2, shuffle=False)
    test_loader31 = DataLoader(torch.load(os.path.join(dataset_dir, 'data_test31.pt')), batch_size=args.batch_size2, shuffle=False)
    test_loader61 = DataLoader(torch.load(os.path.join(dataset_dir, 'data_test61.pt')), batch_size=args.batch_size2, shuffle=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='gno', help='Model identifier.')
    ...
    parser.add_argument('--ntrain', type=int, default=100, help='Number of training samples.')  # Replace 10000 with actual default value
    ...
    parser.add_argument('--ntest', type=int, default=40, help='Number of test samples.')  # Replace 2000 with actual default value
