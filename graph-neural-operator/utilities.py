...
class SquareMeshGenerator(object):
    ...
    def aug_equ_edge_attributes(self, f=None, theta=None):
        if f is None:
            if theta is None:
                edge_attr = None
            else:
...
def get_stats(data_list, args, comp_args,
    ...
    for dp in data_list:
        ...
        if (args.rela_perm == 'none'):
            ...
            gs_rela_perm, _ = calc_rela_perm(args, comp_args, dp.y, 1. - dp.y)
    ...
    if (use_single_dist):
        ...
        # sh is the 3rd element for 2D cases (first two are x and y)
        # sh is the 4th element for 2D cases (first three are x, y, and z)
        mean_vec_x[args.dim] = mean_vec_y
        std_vec_x[args.dim] = std_vec_y
